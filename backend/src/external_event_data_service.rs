...
pub async fn query(
  ...
  let sql = [
    ...
    " WHERE 1 = 1",
    " AND ($1  IS NULL OR eed.external_event_data_id = $1)",
    " AND ($2  IS NULL OR eed.creation_time >= $2)",
    " AND ($3  IS NULL OR eed.creation_time <= $3)",
    " AND ($4  IS NULL OR eed.creator_user_id = $4)",
    " AND ($5  IS NULL OR eed.external_event_id = $5)",
    " AND ($6  IS NULL OR eed.name = $6)",
    " AND ($7  IS NULL OR eed.name LIKE CONCAT('%',$7,'%'))",
    " AND ($8  IS NULL OR eed.start_time >= $8)",
    " AND ($9  IS NULL OR eed.start_time <= $9)",
    " AND ($10 IS NULL OR eed.end_time >= $10)",
    " AND ($11 IS NULL OR eed.end_time <= $11)",
    " AND ($12 IS NULL OR eed.active = $12)",

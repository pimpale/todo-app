...
pub async fn query(
  ...
  let sql = [
    ...
    " WHERE 1 = 1",
    " AND ($1  IS NULL OR gd.goal_data_id = $1)",
    " AND ($2  IS NULL OR gd.creation_time >= $2)",
    " AND ($3  IS NULL OR gd.creation_time <= $3)",
    " AND ($4  IS NULL OR gd.creator_user_id = $4)",
    " AND ($5  IS NULL OR gd.goal_id = $5)",
    " AND ($6  IS NULL OR gd.name = $6)",
    " AND ($7  IS NULL OR gd.tags @> $7)",
    " AND ($8  IS NULL OR gd.name LIKE CONCAT('%',$8,'%'))",
    " AND ($9  IS NULL OR gd.duration_estimate >= $9)",
    " AND ($10 IS NULL OR gd.duration_estimate <= $10)",
    " AND ($11 IS NULL OR gd.time_utility_function_id = $11)",
    " AND ($12 IS NULL OR gd.parent_goal_id = $12 IS TRUE)",
    " AND ($13 IS NULL OR gd.start_time >= $13 IS TRUE)",
    " AND ($14 IS NULL OR gd.start_time <= $14 IS TRUE)",
    " AND ($15 IS NULL OR gd.end_time >= $15 IS TRUE)",
    " AND ($16 IS NULL OR gd.end_time <= $16 IS TRUE)",
    " AND ($17 IS NULL OR gd.status = $17)",
    " AND ($18 IS NULL OR g.goal_intent_id = $18 IS TRUE)",
    " AND ($19 IS NULL OR gd.start_time IS NOT NULL",
